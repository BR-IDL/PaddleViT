...
from utils import RandomResizedCropAndInterpolationWithTwoPic
...
def get_train_transforms_pretrin(config):
    ...
    common_aug_op_list = []
    jitter = (float(config.TRAIN.COLOR_JITTER), ) * 3
    common_aug_op_list.append(transforms.ColorJitter(*jitter))
    common_aug_op_list.append(transforms.RandomHorizontalFlip())
    common_aug_op_list.append(RandomResizedCropAndInterpolationWithTwoPic(
        size=config.DATA.IMAGE_SIZE, second_size=config.DATA.SECOND_IMAGE_SIZE,
        interpolation='bicubic', second_interpolation='lanczos'))

    patch_aug_op_list = []
    patch_aug_op_list.append(transforms.ToTensor())
    patch_aug_op_list.append(transforms.Normalize(mean=config.DATA.IMAGENET_MEAN,
                                                  std=config.DATA.IMAGENET_STD))

    if config.MODEL.VAE_TYPE == 'dall-e':
        visual_token_op_list = []
        visual_token_op_list.append(transforms.ToTensor())
        visual_token_op_list.append(map_pixels)
    elif config.MODEL.VAE_TYPE == 'customized':
        visual_token_op_list = []
        visual_token_op_list.append(transforms.ToTensor())
        visual_token_op_list.append(transforms.Normalize(mean=config.DATA.IMAGENET_MEAN,
                                                         std=config.DATA.IMAGENET_STD))
    else:
        raise NotImplementedError()

    transforms_common = transforms.Compose(common_aug_op_list)
    transforms_patch = transforms.Compose(patch_aug_op_list)
    transforms_token = transforms.Compose(visual_token_op_list)

    return transforms_common, transforms_patch, transforms_token
