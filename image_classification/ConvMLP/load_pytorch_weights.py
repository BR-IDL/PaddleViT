...
def torch_to_paddle_mapping(model_name, config):
    ...
    for stage_idx in range(config.MODEL.MIXER.N_CONV_BLOCKS):
        th_prefix = f'conv_stages.conv_blocks.{stage_idx}'
        pp_prefix = f'conv_stages.conv_blocks.{stage_idx}'

        layer_mapping = [
            (f'{th_prefix}.0', f'{pp_prefix}.0'),
            (f'{th_prefix}.1', f'{pp_prefix}.1'),
            (f'{th_prefix}.3', f'{pp_prefix}.3'),
            (f'{th_prefix}.4', f'{pp_prefix}.4'),
            (f'{th_prefix}.6', f'{pp_prefix}.6'),
            (f'{th_prefix}.7', f'{pp_prefix}.7'),
        ]
        mapping.extend(layer_mapping)
    
    mapping.extend([('conv_stages.downsample', 'conv_stages.downsample')])

    
    mapping.extend([
        ('tokenizer.block.0', 'tokenizer.block.0'),
        ('tokenizer.block.1', 'tokenizer.block.1'),
        ('tokenizer.block.3', 'tokenizer.block.3'),
        ('tokenizer.block.4', 'tokenizer.block.4'),
        ('tokenizer.block.6', 'tokenizer.block.6'),
        ('tokenizer.block.7', 'tokenizer.block.7'),
    ])


    for stage_idx, depth in enumerate(config.MODEL.MIXER.BLOCKS):
        for d in range(depth):
            th_prefix = f'stages.{stage_idx}.blocks.{d}'
            pp_prefix = f'stages.{stage_idx}.blocks.{d}'

            layer_mapping = [
                (f'{th_prefix}.norm1', f'{pp_prefix}.norm1'),
                (f'{th_prefix}.channel_mlp1.fc1', f'{pp_prefix}.channel_mlp1.fc1'),
                (f'{th_prefix}.channel_mlp1.fc2', f'{pp_prefix}.channel_mlp1.fc2'),
                (f'{th_prefix}.norm2', f'{pp_prefix}.norm2'),
                (f'{th_prefix}.connect', f'{pp_prefix}.connect'),
                (f'{th_prefix}.connect_norm', f'{pp_prefix}.connect_norm'),
                (f'{th_prefix}.channel_mlp2.fc1', f'{pp_prefix}.channel_mlp2.fc1'),
                (f'{th_prefix}.channel_mlp2.fc2', f'{pp_prefix}.channel_mlp2.fc2'),
            ]
            mapping.extend(layer_mapping)
        
        if stage_idx <= len(config.MODEL.MIXER.BLOCKS) - 1:
            mapping.extend([(f'stages.{stage_idx}.downsample_mlp.downsample',
                             f'stages.{stage_idx}.downsample_mlp.downsample')])
...
def main():
    ...
    for model_name in model_name_list:
        ...
        torch_model = eval(f'{model_name}()')
        state_dict = torch.load(f'/workspace/model_weights/{model_name}_imagenet.pth')
