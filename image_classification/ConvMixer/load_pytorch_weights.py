...
def torch_to_paddle_mapping(model_name, config):
    ...
    for stage_idx in range(3, config.MODEL.CNN.DEPTH+3):
        th_prefix = f'{stage_idx}'
        pp_prefix = f'{stage_idx}'

        layer_mapping = [
            (f'{th_prefix}.0.fn.0', f'{pp_prefix}.0.fn.0'),
            (f'{th_prefix}.0.fn.2', f'{pp_prefix}.0.fn.2'),
    ...
    head_mapping = [
        (f'{config.MODEL.CNN.DEPTH + 5}', f'{config.MODEL.CNN.DEPTH + 5}'),
    ]
...
def main():
    ...
    for model_name in model_name_list:
        ...
        if '768' in model_name:
            torch_model = ConvMixer(768, 32, 7, 7, act='ReLU')
            state_dict = torch.load(f'/workspace/model_weights/convmixer_pth/{model_name}_ks7_p7_relu.pth.tar')
        elif '1024' in model_name:
            torch_model = ConvMixer(1024, 20, 9, 14)
            state_dict = torch.load(f'/workspace/model_weights/convmixer_pth/{model_name}_ks9_p14.pth.tar')
        elif '1536' in model_name:
            torch_model = ConvMixer(1536, 20, 9, 7)
            state_dict = torch.load(f'/workspace/model_weights/convmixer_pth/{model_name}_ks9_p7.pth.tar')
