...
from cyclemlp_torch import CycleMLP_B5
...
def torch_to_paddle_mapping(model_name, config):
    ...
    for stage_idx, depth in enumerate(config.MODEL.LAYERS):
        for d in range(depth):
            th_prefix = f'network.{stage_idx*2}.{d}'
            pp_prefix = f'network.{stage_idx*2}.{d}'

            layer_mapping = [
                (f'{th_prefix}.norm1', f'{pp_prefix}.norm1'),
                (f'{th_prefix}.attn.mlp_c', f'{pp_prefix}.attn.mlp_c'),
                (f'{th_prefix}.attn.sfc_h', f'{pp_prefix}.attn.sfc_h'),
                (f'{th_prefix}.attn.sfc_h.offset', f'{pp_prefix}.attn.sfc_h.offset'),
                (f'{th_prefix}.attn.sfc_w', f'{pp_prefix}.attn.sfc_w'),
                (f'{th_prefix}.attn.sfc_w.offset', f'{pp_prefix}.attn.sfc_w.offset'),
        ...
        mapping.extend([(f'network.{stage_idx*2-1}.proj', f'network.{stage_idx*2-1}.proj')])
...
def main():
    ...
    for model_name in model_name_list:
        ...
        i = model_name[-1]
        torch_model = eval(f'CycleMLP_B{i}()')
        state_dict = torch.load(f'./cyclemlp_pth/CycleMLP_B{i}.pth')['model']
