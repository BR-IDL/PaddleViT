...
"""DeiT train and eval using multiple GPU """
import sys
import os
import time
import argparse
...
from deit import build_deit as build_deit_distill_model
from deit import build_vit as build_deit_model
...
def main_worker(*args):
    ...
    if config.TRAIN.DISTILLATION_TYPE == 'none':
        model = build_deit_model(config)
    else:
        model = build_deit_distill_model(config)
