...
class Mlp(nn.Layer):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.fc1 = nn.Linear(in_features, hidden_features, weight_attr=weight_attr, bias_attr=bias_attr)
        self.act = act_layer()
        self.fc2 = nn.Linear(hidden_features, out_features, weight_attr=weight_attr, bias_attr=bias_attr)
...
class WindowAttention(nn.Layer):
    ...
    def __init__(self, dim, expand_size, window_size, focal_window, focal_level, num_heads, 
        ...
        if self.expand_size > 0 and focal_level > 0:
            ...
            self.relative_position_bias_table_to_neighbors = paddle.create_parameter(shape=(1, num_heads, self.window_size[0] * self.window_size[1], self.window_size_of_key),
        ...
        if pool_method != "none" and focal_level > 1:
            ...
            for k in range(focal_level-1):
                ...
                relative_position_bias_table_to_windows = paddle.create_parameter(shape=(self.num_heads,
    ...
    def forward(self, x_all, mask_all=None):
        ...
        if self.pool_method != "none" and self.focal_level > 1:
            ...
            for k in range(self.focal_level-1):
                ...
                unfolded_mask = self.unfolds[k](mask.unsqueeze(0).unsqueeze(1)).reshape((
                    1, 1, self.unfolds[k].kernel_sizes[0], self.unfolds[k].kernel_sizes[1], -1)).transpose((0, 4, 2, 3, 1)).\
        ...
        attn = (paddle.mm(q_windows, k_all.transpose((0, 1, 3, 2))))  # B*nW, nHead, window_size*window_size, focal_window_size*focal_window_size

        window_area = self.window_size[0] * self.window_size[1]        
        window_area_rolled = k_rolled.shape[2]

        # add relative position bias for tokens inside window
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.flatten()].reshape((
        ...
        if self.pool_method != "none" and self.focal_level > 1:
            ...
            for k in range(self.focal_level-1):
                ...
                relative_position_bias_to_windows = paddle.gather(relative_position_bias_to_windows, relative_position_index_k.flatten(), axis=1).reshape((
                ...
                if mask_all[k+1] is not None:
                    attn[:, :, :window_area, offset:(offset + (self.focal_window+2**k-1)**2)] = \
                                    attn[:, :, :window_area, offset:(offset + (self.focal_window+2**k-1)**2)] + \
                                    paddle.stack([mask_all[k+1].unsqueeze(-2).unsqueeze(-2)] * (attn.shape[0] // mask_all[k+1].shape[1]), axis=0).\
        ...
        if mask_all[0] is not None:
            ...
            attn[:, :, :, :, :window_area] = attn[:, :, :, :, :window_area] + mask_all[0].unsqueeze(0).unsqueeze(2)
        ...
        x = paddle.mm(attn, v_all).transpose((0, 2, 1, 3)).reshape((attn.shape[0], window_area, C))
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class FocalTransformerBlock(nn.Layer):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, expand_size=0, shift_size=0,
        ...
        if self.pool_method != "none":
            for k in range(self.focal_level-1):
                ...
                if self.pool_method == "fc":
                    self.pool_layers.append(nn.Linear(window_size_glo * window_size_glo, 1, weight_attr=weight_attr, bias_attr=bias_attr))
                    self.pool_layers[len(self.pool_layers)-1].weight.set_value(
                        paddle.full_like(self.pool_layers[len(self.pool_layers)-1].weight, 1./(window_size_glo * window_size_glo))
                    )
        ...
        self.norm1 = norm_layer(dim, weight_attr=paddle.ParamAttr(initializer=nn.initializer.Constant(1.0)), bias_attr=bias_attr)
        ...
        self.norm2 = norm_layer(dim, weight_attr=paddle.ParamAttr(initializer=nn.initializer.Constant(1.0)), bias_attr=bias_attr)
        ...
        if self.focal_level > 1 and self.pool_method != "none": 
            ...
            for k in range(self.focal_level-1):     
                ...
                if self.pool_method == "mean":
                    ...
                    x_windows_pooled = x_windows_noreshape.max(-2)[0].max(-2)[0].reshape((B, nWh, nWw, C)) # B, nWh, nWw, C                    
                elif self.pool_method == "fc":
                    x_windows_noreshape = x_windows_noreshape.reshape((B, nWh, nWw, window_size_glo*window_size_glo, C)).transpose((0, 1, 2, 4, 3)) # B, nWh, nWw, C, wsize**2
                    x_windows_pooled = self.pool_layers[k](x_windows_noreshape).flatten(-2) # B, nWh, nWw, C                      
                elif self.pool_method == "conv":
                    x_windows_noreshape = x_windows_noreshape.reshape((-1, window_size_glo, window_size_glo, C)).transpose((0, 3, 1, 2)) # B * nw * nw, C, wsize, wsize
                    x_windows_pooled = self.pool_layers[k](x_windows_noreshape).reshape((B, nWh, nWw, C)) # B, nWh, nWw, C           
...
class PatchMerging(nn.Layer):
    ...
    def __init__(self, img_size, in_chans=3, norm_layer=nn.LayerNorm, **kwargs):
        ...
        self.norm = norm_layer(4 * in_chans, weight_attr=paddle.ParamAttr(initializer=nn.initializer.Constant(1.0)), bias_attr=bias_attr)
...
class PatchEmbed(nn.Layer):
    ...
    def __init__(self, img_size=(224, 224), patch_size=4, in_chans=3, embed_dim=96, 
        ...
        if norm_layer is not None:
            self.norm = norm_layer(embed_dim, weight_attr=paddle.ParamAttr(initializer=nn.initializer.Constant(1.0)), bias_attr=bias_attr)
        else:
            self.norm = None
...
class FocalTransformer(nn.Layer):
    ...
    def __init__(self, 
        ...
        self.patch_embed = PatchEmbed(
            img_size=(img_size, img_size), patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim, use_conv_embed=use_conv_embed, is_stem=True, 
        ...
        if self.ape:
            self.absolute_pos_embed = paddle.create_parameter(shape=(1, num_patches, embed_dim), 
                                                              dtype=np.int32, is_bias=True, attr=nn.initializer.TruncatedNormal(std=.02))

        self.pos_drop = nn.Dropout(p=drop_rate)

        # stochastic depth
        dpr = [x.numpy().item() for x in paddle.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule
        ...
        self.norm = norm_layer(self.num_features, weight_attr=paddle.ParamAttr(initializer=nn.initializer.Constant(1.0)), bias_attr=bias_attr)
        self.avgpool = nn.AdaptiveAvgPool1D(1)
        self.head = nn.Linear(self.num_features, num_classes, weight_attr=weight_attr, bias_attr=bias_attr) if num_classes > 0 else Identity()
