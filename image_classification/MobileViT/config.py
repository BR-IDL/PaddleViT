...
# data settings
_C.DATA = CN()
_C.DATA.BATCH_SIZE = 128 #256 # train batch_size for single GPU
...
_C.DATA.IMAGE_SIZE = 256 # input image size: 224 for pretrain, 384 for finetune
...
_C.MODEL.DIMS = [144,192,240]
_C.MODEL.CHANNELS = [16, 32, 64, 64, 96, 96, 128, 128, 160, 160, 640]
_C.MODEL.MV2_EXPAANSION = 4
...
_C.TRAIN.WARMUP_EPOCHS = 100 #34 # ~ 10k steps for 4096 batch size
_C.TRAIN.WEIGHT_DECAY = 0.01 #0.3 # 0.0 for finetune
_C.TRAIN.BASE_LR = 0.9e-3 #0.003 for pretrain # 0.03 for finetune
_C.TRAIN.WARMUP_START_LR = 0.9e-4 #0.0
_C.TRAIN.END_LR = 0.9e-4
