...
def torch_to_paddle_mapping(model_name, config):
    ...
    layer_ids = [x for x in range(len(config.MODEL.LAYERS) * 2 - 1)]  # [0,1,2,3,4,5,6]

    for stage_idx, stage_depth in enumerate(config.MODEL.LAYERS):
        for d in range(stage_depth):
            pp_prefix = f'network.{stage_idx*2}.{d}'
            th_prefix = f'network.{stage_idx*2}.{d}'

            layer_mapping = [
                (f'{th_prefix}.layer_scale_1', f'{pp_prefix}.layer_scale_1'),
                (f'{th_prefix}.layer_scale_2', f'{pp_prefix}.layer_scale_2'),
