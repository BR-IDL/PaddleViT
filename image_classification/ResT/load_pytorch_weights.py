...
def torch_to_paddle_mapping(model_name, config):
    ...
    for idx in range(3):
        layer_mapping = [
            (f'patch_embed_{idx+2}.conv', f'patch_embeds.{idx}.conv'),
            (f'patch_embed_{idx+2}.norm', f'patch_embeds.{idx}.norm'),
            (f'patch_embed_{idx+2}.pos.pa_conv', f'patch_embeds.{idx}.pos.pa_conv'),
        ]
        mapping.extend(layer_mapping)
    ...
    for stage_idx, depth in enumerate(config.MODEL.DEPTHS):
        for block_idx in range(depth):
            ...
            layer_mapping = [
                ...
                (f'{th_prefix}.attn.transform_conv', f'{pp_prefix}.attn.transform_conv'),
...
def main():
    ...
    for model_name in model_name_list:
        ...
        torch_model.load_state_dict(torch.load(f'./rest_pth/{model_name}.pth'))
        torch_model.eval()
