...
def torch_to_paddle_mapping(model_name, config):
    mapping = []
    layer_mapping = [
        ('stem.proj.0', 'stem.proj.0'),
        ('stem.proj.1', 'stem.proj.1'),
        ('stem.proj.3', 'stem.proj.3'),
        ('stem.proj.4', 'stem.proj.4'),
        ('stem.proj.6', 'stem.proj.6'),
        ('stem.pos.pa_conv', 'stem.pos.pa_conv'),
        ('stem.norm', 'stem.norm'),
    ]
    ...
    for idx in range(3):
        layer_mapping = [
            (f'patch_{idx+2}.proj', f'patch_embeds.{idx}.proj'),
            (f'patch_{idx+2}.norm', f'patch_embeds.{idx}.norm'),
            (f'patch_{idx+2}.pos.pa_conv', f'patch_embeds.{idx}.pos.pa_conv'),
        ]
        mapping.extend(layer_mapping)

    for stage_idx, depth in enumerate(config.MODEL.DEPTHS):
        for block_idx in range(depth):
            ...
            layer_mapping = [
                ...
                (f'{th_prefix}.attn.sr_norm', f'{pp_prefix}.attn.sr_norm'),
                (f'{th_prefix}.attn.proj', f'{pp_prefix}.attn.proj'),
                (f'{th_prefix}.attn.up.0', f'{pp_prefix}.attn.up.0'),
                (f'{th_prefix}.attn.up_norm', f'{pp_prefix}.attn.up_norm'),
...
def main():
    ...
    for model_name in model_name_list:
        ...
        torch_model.load_state_dict(torch.load(f'./rest_pth/{model_name}_224.pth')['model'])
        torch_model.eval()
