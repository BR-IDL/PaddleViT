...
class Attention(nn.Layer):
    def __init__(self,
        ...
        if sr_ratio > 1:
            ...
            self.sr_norm = nn.LayerNorm(embed_dim, weight_attr=w_attr, bias_attr=b_attr, epsilon=1e-5)

        self.apply_transform = apply_transform and num_heads > 1
        if self.apply_transform:
            self.transform_conv = nn.Conv2D(self.num_heads,
                                            self.num_heads,
                                            kernel_size=1,
                                            stride=1)
            self.transform_norm = nn.InstanceNorm2D(self.num_heads, weight_attr=False, bias_attr=False)
    ...
    def forward(self, x, H, W):
        ...
        #if self.sr_ratio > 1:
        #    x_ = x.transpose([0, 2, 1])  # [B, N, C] -> [B, C, N]
        #    x_ = x_.reshape([B, C, H, W])  # [B, C, N] -> [B, C, H, W]
        #    x_ = self.sr(x_)
        #    x_ = x_.reshape([B, C, -1])  # [B, C, H, W] -> [B, C, N]
        #    x_ = x_.transpose([0, 2, 1])  # [B, C, N] -> [B, N, C]
        #    x_ = self.sr_norm(x_)
        #    kv = self.kv(x_)
        ...
        #else:
        #    kv = self.kv(x)
        #    kv = kv.reshape([B, N, 2, self.num_heads, self.attn_head_size])
        #    kv = kv.transpose([2, 0, 3, 1, 4])
...
class Block(nn.Layer):
    def __init__(self,
        ...
        super().__init__()
        w_attr, b_attr = self._init_weights_norm()
        self.norm1 = nn.LayerNorm(dim, weight_attr=w_attr, bias_attr=b_attr, epsilon=1e-5)
        ...
        w_attr, b_attr = self._init_weights_norm()
        self.norm2 = nn.LayerNorm(dim, weight_attr=w_attr, bias_attr=b_attr, epsilon=1e-5)
...
class BasicStem(nn.Layer):
    def __init__(self, in_channels=3, out_channels=64, with_pos=False):
        super().__init__()
...
class ResT(nn.Layer):
    def __init__(self,
        ...
        # Stem Layers
        self.stem = BasicStem(in_channels=in_channels,
        ...
        w_attr, b_attr = self._init_weights_norm()
        self.norm = nn.LayerNorm(embed_dims[3], weight_attr=w_attr, bias_attr=b_attr, epsilon=1e-5)
        # Head
...
def build_rest(config):
    """build rest model from config"""
    model = ResT(in_channels=config.DATA.IMAGE_CHANNELS,
                 ...
                 apply_transform=config.MODEL.APPLY_TRANSFORM)
    return model
