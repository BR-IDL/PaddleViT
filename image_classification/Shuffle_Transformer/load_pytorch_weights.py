...
def torch_to_paddle_mapping(model_name, config):
    ...
    for stage_idx, num_layers in enumerate(config.MODEL.STAGE_DEPTHS):
...
def main():
    ...
    model_name_list = ['shuffle_vit_tiny_patch4_window7_224',
    ...
    for model_name in model_name_list:
        ...
        if 'tiny' in model_name:
            torch_model = ShuffleTransformerTorch(layers=[2, 2, 6, 2],
                                                  ...
                                                  qkv_bias=True,
                                                  embed_dim=96)
            model_state_dict = torch.load('./shuffle_vit_tiny_patch4_window7_224_ep298.pth', map_location='cpu') 
        elif 'small' in model_name:
            torch_model = ShuffleTransformerTorch(layers=[2, 2, 18, 2],
                                                  num_heads=[3, 6, 12, 24],
                                                  qkv_bias=True,
                                                  embed_dim=96)
            model_state_dict = torch.load('./shuffle_vit_small_patch4_window7_224_ep292.pth', map_location='cpu') 
        elif 'base' in model_name:
            torch_model = ShuffleTransformerTorch(layers=[2, 2, 18, 2],
                                                  num_heads=[4, 8, 16, 32],
                                                  qkv_bias=True,
                                                  embed_dim=128)
            model_state_dict = torch.load('./shuffle_vit_base_patch4_window7_224_ep296.pth', map_location='cpu') 
        else:
            print('Wrong name')
