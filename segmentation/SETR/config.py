...
# data settings
_C.DATA = CN()
_C.DATA.BATCH_SIZE = 2 #256 # train batch_size for single GPU
_C.DATA.BATCH_SIZE_VAL = 16 #64 # val batch_size for single GPU
_C.DATA.DATA_PATH = '/dataset/imagenet/' # path to dataset
_C.DATA.DATASET = 'imagenet2012' # dataset name
_C.DATA.IMAGE_SIZE = 384 # input image size: 224 for pretrain, 384 for finetune
...
_C.MODEL.TRANS.HIDDEN_SIZE = 512
_C.MODEL.TRANS.MLP_RATIO = 4.
_C.MODEL.TRANS.NUM_HEADS = 8
_C.MODEL.TRANS.NUM_LAYERS = 24
_C.MODEL.TRANS.QKV_BIAS = False


_C.MODEL.MLA = CN()
_C.MODEL.MLA.MLA_CHANNELS = 256
_C.MODEL.MLA.MLA_INDEX = [5, 11, 17, 23]

# training settings
_C.TRAIN = CN()
_C.TRAIN.LAST_EPOCH = 0
_C.TRAIN.NUM_EPOCHS = 300
_C.TRAIN.WARMUP_EPOCHS = 34 # ~ 10k steps for 4096 batch size
_C.TRAIN.WEIGHT_DECAY = 0.3 # 0.0 for finetune
