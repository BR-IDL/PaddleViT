...
class VIT_MLA_AUXIHead(nn.Layer):
    ...
    def __init__(self, in_channels = 256, num_classes = 60, align_corners= False):
        ...
        if self.in_channels == 1024:
            self.aux_0 = nn.Conv2D(self.in_channels, 256, kernel_size=1, bias_attr=False)
            self.aux_1 = nn.Conv2D(256, self.num_classes, kernel_size=1, bias_attr=False)
        elif self.in_channels == 256:
            self.aux = nn.Conv2D(self.in_channels, self.num_classes, kernel_size=1, bias_attr=False)
    ...
    def forward(self, x):
        ...
        if self.in_channels == 1024:
            ...
            aux_pred = self.aux(x)
        aux_pred_full = F.interpolate(aux_pred, up16x_resolution, mode='bilinear', align_corners=self.align_corners)
